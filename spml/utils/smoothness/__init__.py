...
def get_saliency_smoothness(pred, gt, size_average=True):
    ...
    gt_y = gradient_y(gt)
    # for visualization
    '''np_gx = gt_x[0].cpu().squeeze().detach().numpy()
    np_gy = gt_y[0].cpu().squeeze().detach().numpy()
    np_sal_x = sal_x[0].cpu().squeeze().detach().numpy()
    np_sal_y = sal_y[0].cpu().squeeze().detach().numpy()
    plt.subplot(221)
    plt.imshow(np_gx, cmap='gray')
    plt.title('GT Gradient X')
    plt.axis('off')
    plt.subplot(222)
    plt.imshow(np_gy, cmap='gray')
    plt.title('GT Gradient Y')
    plt.axis('off')
    plt.subplot(223)
    plt.imshow(np_sal_x, cmap='gray')
    plt.title('Pred Gradient X')
    plt.axis('off')
    plt.subplot(224)
    plt.imshow(np_sal_y, cmap='gray')
    plt.title('Pred Gradient Y')'''
