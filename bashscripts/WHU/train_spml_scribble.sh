...
TRAIN_DATA_LIST=datasets/WHU/scribble_${TRAIN_SPLIT}.txt   # <-- 如使用自有数据集，请替换为你的 list 文件
VALID_DATA_LIST=datasets/WHU/panoptic_${VALID_SPLIT}.txt   # <-- 如使用自有数据集，请替换为你的 val list
TEST_DATA_LIST=datasets/WHU/panoptic_${INFERENCE_SPLIT}.txt       # <-- 如使用自有数据集，请替换为你的 test list
MEMORY_DATA_LIST=datasets/WHU/panoptic_${TRAIN_SPLIT}.txt     # <-- 如无 memory 数据，请指向合适的 list 或空文件
