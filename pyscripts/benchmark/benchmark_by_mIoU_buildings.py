...
def iou_stats(pred, target, num_classes=2, background=0):
...
def main():
  ...
  recall = tp[1] / (tp_fn[1] + 1e-12)
  precision = tp[1] / (tp_fp[1] + 1e-12)
  f1_score = 2 * recall * precision / (recall + precision + 1e-12)
  print('Building Recall: {:4.4f}%'.format(recall * 100.0))
  print('Building Precision: {:4.4f}%'.format(precision * 100.0))
  print('Building F1 Score: {:4.4f}%'.format(f1_score * 100.0)) 
