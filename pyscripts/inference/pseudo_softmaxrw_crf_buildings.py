...
def main():
  ...
  with torch.no_grad():
    for data_index in tqdm(range(len(test_dataset))):
      ...
      label_tags = np.zeros((config.dataset.num_classes,), dtype=bool)
      ...
      # normalize cam.
      max_prob = torch.max(semantic_probs.view(2, -1), dim=1)[0]
      cam_full_arr = semantic_probs / max_prob.view(2, 1, 1)
      ...
      cam_rw = cam_rw.view(2, cam_shape[0], cam_shape[1])
