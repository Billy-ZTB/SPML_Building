...
def main():
  ...
  # Create data loaders.
  test_dataset = ContrastiveScribbleBuildingDataset(
  ...
  elif config.network.prediction_types == 'segsort_buildings':
  ...
  state = torch.load(model_path_template.format(save_iter))
  # 如果你保存的 checkpoint 里有 'embedding_model' 键就取它，否则假设 checkpoint 就是 state_dict
  if config.network.backbone_types == 'vgg16_deeplab':
    state_dict = state['embedding_model'] if 'embedding_model' in state else state
    ...
    with torch.no_grad():
      for ind_h in patch_ind_h:
        for ind_w in patch_ind_w:
          ...
          # Feed-forward.
          if config.network.backbone_types == 'vgg16_deeplab':
            crop_embeddings = embedding_model.forward(
                crop_image_batch)
          else:
