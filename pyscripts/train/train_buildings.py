...
from spml.data.datasets.buildings_dataset import ContrastiveScribbleBuildingDataset
...
from spml.models.predictions.segsort_buildings import segsort_buildings
from spml.models.predictions.softmax_classifier import softmax_classifier
from spml.models.embeddings.vgg_deeplab_model import *
from spml.utils.metrics import Evaluator
...
def main():
  ...
  # Create data loaders.
  train_dataset = ContrastiveScribbleBuildingDataset(
  ...
  elif config.network.backbone_types == 'vgg16_deeplab':
    embedding_model = VGG_Contrastive(config).cuda()
  else:
    raise ValueError('Not support ' + config.network.backbone_types)

  if config.network.prediction_types == 'segsort_buildings':
    prediction_model = segsort_buildings(config).cuda()
  elif config.network.prediction_types == 'segsort':
  ...
  for curr_iter in pbar:
    ...
    for i in range(len(label_batch)):
      ...
      #print(semantic_tags[i].dtype, label_batch[i]['prototype_batch_index'].dtype)
    ...
    for k in ['sem_ann_loss', 'sem_occ_loss', 'img_sim_loss', 'feat_aff_loss', 
               'sal2_smoothness',  'sal2_seg_loss', 'edge_loss']:
    ...
    '''loss_names = ['sem_ann_loss', 'img_sim_loss', 'sem_occ_loss',
            'sal1_smoothness', 'sal2_smoothness', 'sal1_seg_loss',
            'sal2_seg_loss', 'edge_loss']'''
    loss_names = ['sem_ann_loss', 'img_sim_loss', 'sem_occ_loss']
    parts = ['total={:.3f}'.format(loss.item())]
    for name in loss_names:
      v = outputs.get(name, None)
      parts.append('{}={:.3f}'.format(name, v.item() if v is not None else 0.0))
    parts.append('acc={:.3f}'.format(acc.item()))
